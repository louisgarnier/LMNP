"""
Migration: Add property_id to loan_payments table.

This script adds the property_id column to the loan_payments table
...
def migrate():
    """Add property_id to loan_payments table."""
    ...
    try:
        print("=== Ajout de property_id à loan_payments ===\n")
        ...
        if 'property_id' in columns:
            ...
            cursor.execute("SELECT COUNT(*) FROM loan_payments WHERE property_id IS NULL")
            null_count = cursor.fetchone()[0]
            
            if null_count > 0:
                print(f"⚠️  {null_count} loan_payment(s) ont property_id=NULL")
                ...
                cursor.execute(f"UPDATE loan_payments SET property_id = {default_property_id} WHERE property_id IS NULL")
                conn.commit()
                print(f"✅ {null_count} loan_payment(s) mis à jour avec property_id={default_property_id}")
            else:
                print("✅ Tous les loan_payments ont déjà un property_id")
        else:
            ...
            # Compter les loan_payments existants
            cursor.execute("SELECT COUNT(*) FROM loan_payments")
            payments_count = cursor.fetchone()[0]
            print(f"   {payments_count} loan_payment(s) existant(s) à mettre à jour")
            
            # Ajouter la colonne property_id (sans NOT NULL d'abord)
            cursor.execute("ALTER TABLE loan_payments ADD COLUMN property_id INTEGER")
            
            # Assigner property_id par défaut à tous les loan_payments existants
            if payments_count > 0:
                cursor.execute(f"UPDATE loan_payments SET property_id = {default_property_id} WHERE property_id IS NULL")
                print(f"✅ {payments_count} loan_payment(s) mis à jour avec property_id={default_property_id}")
            
            conn.commit()
            print("✅ Colonne property_id ajoutée avec succès")
        
        # 2. Vérifier/Créer l'index sur property_id
        print("\n📋 Vérification de l'index idx_loan_payments_property_id...")
        cursor.execute("PRAGMA index_list(loan_payments)")
        indexes = [idx[1] for idx in cursor.fetchall()]
        
        if 'idx_loan_payments_property_id' in indexes:
            print("✅ Index idx_loan_payments_property_id présent")
        else:
            print("📋 Création de l'index idx_loan_payments_property_id...")
            cursor.execute("""
                CREATE INDEX idx_loan_payments_property_id 
                ON loan_payments(property_id)
            """)
            conn.commit()
            print("✅ Index créé")
        ...
        null_count = cursor.fetchone()[0]
        if null_count > 0:
            print(f"❌ ERREUR: {null_count} loan_payment(s) ont encore property_id=NULL")
            return False
        
        cursor.execute("SELECT COUNT(*) FROM loan_payments")
        total = cursor.fetchone()[0]
        print(f"✅ Total loan_payments: {total}")
