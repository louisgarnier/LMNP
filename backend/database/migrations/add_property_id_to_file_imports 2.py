"""
Migration: Add property_id to file_imports table.

This script adds the property_id column to the file_imports table,
assigns a default property_id to existing file imports,
...
def migrate():
    """Add property_id to file_imports table."""
    ...
    try:
        print("=== Ajout de property_id à la table file_imports ===\n")
        
        # 1. Vérifier si la table file_imports existe
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='file_imports'")
        if not cursor.fetchone():
            print("❌ ERREUR: La table file_imports n'existe pas")
            return False
        
        # 2. Vérifier si property_id existe déjà
        cursor.execute("PRAGMA table_info(file_imports)")
        columns = [row[1] for row in cursor.fetchall()]
        
        if 'property_id' in columns:
            print("✅ La colonne property_id existe déjà dans la table file_imports")
            
            # Vérifier s'il y a des imports sans property_id
            cursor.execute("SELECT COUNT(*) FROM file_imports WHERE property_id IS NULL")
            ...
            if null_count > 0:
                ...
                cursor.execute(f"UPDATE file_imports SET property_id = {default_property_id} WHERE property_id IS NULL")
                conn.commit()
            ...
            cursor.execute("SELECT COUNT(*) FROM file_imports")
            ...
            cursor.execute("ALTER TABLE file_imports ADD COLUMN property_id INTEGER")
        ...
        print("\n📋 Vérification de l'index idx_file_imports_property_id...")
        cursor.execute("PRAGMA index_list(file_imports)")
        indexes = [idx[1] for idx in cursor.fetchall()]
        
        if 'idx_file_imports_property_id' in indexes:
            print("✅ Index idx_file_imports_property_id existe déjà")
        else:
            print("📋 Création de l'index idx_file_imports_property_id...")
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_file_imports_property_id 
                ON file_imports(property_id)
            """)
            conn.commit()
            print("✅ Index idx_file_imports_property_id créé")
        
        # 4. Vérifier/Créer l'index unique sur (property_id, filename)
        print("\n📋 Vérification de l'index unique idx_file_imports_property_filename_unique...")
        if 'idx_file_imports_property_filename_unique' in indexes:
            print("✅ Index unique idx_file_imports_property_filename_unique existe déjà")
        else:
            ...
            if duplicates:
                ...
                print("📋 Création de l'index unique idx_file_imports_property_filename_unique...")
                cursor.execute("""
                    CREATE UNIQUE INDEX IF NOT EXISTS idx_file_imports_property_filename_unique 
                    ON file_imports(property_id, filename)
                """)
                conn.commit()
                print("✅ Index unique idx_file_imports_property_filename_unique créé")
        
        # 5. Supprimer l'ancien index unique sur filename seul (s'il existe)
        print("\n📋 Vérification de l'ancien index unique sur filename...")
        cursor.execute("PRAGMA index_list(file_imports)")
        indexes = [idx[1] for idx in cursor.fetchall()]
        
        # SQLite crée automatiquement un index unique pour les colonnes UNIQUE
        # On ne peut pas le supprimer directement, mais il sera remplacé par le nouveau
        
        # 6. Vérifier la contrainte de clé étrangère
        print("\n📋 Vérification de la contrainte FOREIGN KEY...")
        cursor.execute("PRAGMA foreign_key_list(file_imports)")
        fks = cursor.fetchall()
        ...
        if has_fk:
            ...
            print("   ✅ Le modèle FileImport a déjà ForeignKey(), c'est suffisant")
