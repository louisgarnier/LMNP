"""Étape 2 Task 4 : toute écriture d'enrichissement synchronise category_id.
...
(`mapping_obligatoire_service.reset_allowed_mappings`, write site #2 du
brief), et non `update_transaction_classification(level_1=None)` : lecture du
code réel montre que cette dernière traite `level_1=None` (et idem level_2/
level_3) comme « ne pas toucher ce champ » (garde `if level_1 is not None`),
pas comme « effacer » — y compris quand les trois niveaux sont None
simultanément. Il n'existe donc aujourd'hui aucun chemin atteignable pour
désassigner une classification EXISTANTE via cette fonction ; seule la
suppression complète de la ligne `enriched_transactions` (reset en masse ou
DELETE transaction) désassigne réellement. Voir task-4-report.md.
"""
...
def test_reset_allowed_mappings_clears_category_id(db_session):
    ...
    quand une combinaison devient interdite, le Mapping et l'EnrichedTransaction
    associés sont supprimés -> transactions.category_id doit repasser à NULL."""
    from backend.database.models import Property, Transaction, Mapping, AllowedMapping, EnrichedTransaction
    from backend.api.services.enrichment_service import enrich_transaction
    ...
    enriched = db_session.query(EnrichedTransaction).filter(
